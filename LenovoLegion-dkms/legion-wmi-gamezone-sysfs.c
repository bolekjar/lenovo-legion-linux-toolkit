...
static int legion_wmi_validate_input(const u32 value,const u32 *possible_inputs,const size_t possible_inputs_len) {
...
#define LEGION_WMI_KOBJ_ATTR_RO_CACHED(_name,_attr,_func) \
...
static struct kobj_attribute kobj_attr_##_func  = LEGION_WMI_ATTR_RO(_name,_func);


#define LEGION_WMI_KOBJ_ATTR_RO(_name,_attr,_func) \
static ssize_t func_##_func##_show(struct kobject *kobj,struct kobj_attribute *kattr, char *buf) \
{																												 \
	u32 value;																									 \
	struct lenovo_wmi_gz_priv *priv = dev_get_drvdata(kobj_to_dev(kobj->parent));								 \
...
#define LEGION_WMI_KOBJ_ATTR_WO(_name,_attr,_func,_possible_inputs) \
...
static struct kobj_attribute kobj_attr_##_func  = LEGION_WMI_ATTR_WO(_name,_func);

#define LEGION_WMI_KOBJ_ATTR_RW(_name,_attrget,_attrset,_func,_possible_inputs)     \
...
static struct kobj_attribute kobj_attr_##_func  =                                                  \
		LEGION_WMI_ATTR_RW(_name,_func);



#define LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED(_name,_attr,_attrget,_attrset,_dispname,_possible_inputs) \
...
static struct kobj_attribute kobj_attr_##_name  = LEGION_WMI_ATTR_RO(supported,_name);		            	 \
static struct kobj_attribute kobj_attr_display_name_##_name  =                                                   \
		LEGION_WMI_ATTR_RO(display_name,_name##_display_name);	     		 								 \
static struct kobj_attribute kobj_attr_current_value_##_name  =                                                  \
		LEGION_WMI_ATTR_RW(current_value,_name);	     				 						 			     \
																												 \
...
#define LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED_SMARTFAN(_name,_attr,_attrget,_attrset,_dispname,_possible_inputs) \
static ssize_t func_##_name##_show(struct kobject *kobj,struct kobj_attribute *kattr, char *buf) \
{																												 \
	struct lenovo_wmi_gz_priv *priv = dev_get_drvdata(kobj_to_dev(kobj->parent));								 \
																												 \
...
static struct kobj_attribute kobj_attr_extreme_supported_value_##_name  =                                        \
		LEGION_WMI_ATTR_RO(extreme_supported,_name##_extreme_supported);	     				 						 			 \
																												 \
static struct attribute *legion_sysfs_##_name##_attributes[] = {                                                 \
	&kobj_attr_##_name.attr,																		             \
	&kobj_attr_display_name_##_name.attr,																		 \
...
LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED_SMARTFAN(smart_fan,IsSupportSmartFan,GetSmartFanMode,SetSmartFanMode,"Smart Fan feature",SmartFanPossibleValues);
LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED(igpu_mode,IsSupportIGPUMode,GetIGPUModeStatus,SetIGPUModeStatus,"IGPU mode",IGPUModeStates);
LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED(disable_win_key,IsSupportDisableWinKey,GetWinKeyStatus,SetWinKeyStatus,"EC disable/enable windows key capability",StateONOFF);
LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED(disable_tp,IsSupportDisableTP,GetTPStatus,SetTPStatus,"EC disable/enable touchpad capability",StateONOFF);
LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED(gsync,IsSupportGSync,GetGSyncStatus,SetGSyncStatus,"G-Sync feature",StateONOFF);
LEGION_WMI_RO_FEATURE_SET_GET_SUPPORTED(over_drive,IsSupportOD,GetODStatus,SetODStatus,"Over Drive feature capability",StateONOFF);
...
////////////////// Fan related //////////////////////////////////////////
LEGION_WMI_KOBJ_ATTR_RO_CACHED(count,GetFanCount,fan_count);
LEGION_WMI_KOBJ_ATTR_RO(fan1_speed,GetFan1Speed,fan1_speed);
LEGION_WMI_KOBJ_ATTR_RO(fan2_speed,GetFan2Speed,fan2_speed);
LEGION_WMI_KOBJ_ATTR_RO(max_speed,GetFanMaxSpeed,fan_max_speed);
LEGION_WMI_KOBJ_ATTR_RO_STATIC_STRING(display_name,"Others Fan related features",display_fan)
...
static struct kobj_attribute kobj_attr_dgpu_hwid_name  = LEGION_WMI_ATTR_RO(dgpu_hwid,dgpu_hwid);

LEGION_WMI_KOBJ_ATTR_RO(temp,GetGPUTemp,gpu_temp);
LEGION_WMI_KOBJ_ATTR_RO(pow,GetGPUPow,gpu_pow);
LEGION_WMI_KOBJ_ATTR_RO(oc_pow,GetGPUOCPow,gpu_oc_pow);
LEGION_WMI_KOBJ_ATTR_RO(oc_type,GetGPUOCType,gpu_oc_type);
LEGION_WMI_KOBJ_ATTR_WO(notify_dgpu_satus,NotifyDGPUStatus,gpu_notify_dgpu_satus,StateGPUONOFF);
LEGION_WMI_KOBJ_ATTR_RO_STATIC_STRING(display_name,"Others GPU related features",display_gpu);
...
LEGION_WMI_KOBJ_ATTR_RO(temp,GetCPUTemp,cpu_temp);
LEGION_WMI_KOBJ_ATTR_RO(freq,GetCpuFrequency,cpu_freq);
LEGION_WMI_KOBJ_ATTR_RO_STATIC_STRING(display_name,"Others CPU related features",display_cpu);
...
////////////////// Macrokey related //////////////////////////////////////
LEGION_WMI_KOBJ_ATTR_RO(scancode,GetMacrokeyScancode,macro_key_scancode);
LEGION_WMI_KOBJ_ATTR_RO(count,GetMacrokeyCount,macro_key_count);
LEGION_WMI_KOBJ_ATTR_RO_STATIC_STRING(display_name,"Macrokey related features",display_macro_key);
...
LEGION_WMI_KOBJ_ATTR_RO(get_ir_temp,GetPowerChargeMode,get_ir_temp);
LEGION_WMI_KOBJ_ATTR_RO(get_power_charge_mode,GetPowerChargeMode,get_power_charge_mode);
LEGION_WMI_KOBJ_ATTR_RO(get_product_info,GetProductInfo,get_product_info);
LEGION_WMI_KOBJ_ATTR_RO(get_version,GetVersion,get_version);
LEGION_WMI_KOBJ_ATTR_RO(get_trigger_temperature_value,GetTriggerTemperatureValue,get_trigger_temperature_value);
LEGION_WMI_KOBJ_ATTR_RO(get_keyboard_feature_list,GetKeyboardfeaturelist,get_keyboard_feature_list);
LEGION_WMI_KOBJ_ATTR_RO(get_memory_oc_info,GetMemoryOCInfo,get_memory_oc_info);

LEGION_WMI_KOBJ_ATTR_WO(set_light_control_owner,SetLightControlOwner,set_light_control_owner,StateOtherONOFF);
LEGION_WMI_KOBJ_ATTR_WO(set_dds_control_owner,SetDDSControlOwner,set_dds_control_owner,StateOtherONOFF);

LEGION_WMI_KOBJ_ATTR_RW(get_intelligent_sub_mode,GetIntelligentSubMode,SetIntelligentSubMode,get_intelligent_sub_mode,StateOtherONOFF);
LEGION_WMI_KOBJ_ATTR_RO(get_hardware_info_support_version,GetHardwareInfoSupportVersion,get_hardware_info_support_version);
LEGION_WMI_KOBJ_ATTR_RO(get_learning_profile_count,GetLearningProfileCount,get_learning_profile_count);
LEGION_WMI_KOBJ_ATTR_RO(is_ac_fit_for_oc,IsACFitForOC,is_ac_fit_for_oc);
LEGION_WMI_KOBJ_ATTR_RO(get_thermal_mode,GetThermalMode,get_thermal_mode);
LEGION_WMI_KOBJ_ATTR_RO(is_changed_ylog,IsChangedYLog,is_changed_ylog);
LEGION_WMI_KOBJ_ATTR_RO_STATIC_STRING(display_other,"No related features",display_other);
