...
public:
    ...
    BigInteger(     uint32_t  x);
    BigInteger(      int32_t  x);
    BigInteger(     uint16_t  x);
    BigInteger(      int16_t  x);
...
inline BigInteger BigInteger::operator /(const BigInteger &x) const {
    if (x.isZero())
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::operator /: division by zero")
	BigInteger q, r;
	r = *this;
