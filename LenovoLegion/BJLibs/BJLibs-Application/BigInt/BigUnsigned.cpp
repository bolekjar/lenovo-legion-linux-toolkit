...
uint64_t       BigUnsigned::toUnsignedInt64() const { return convertToPrimitive      <uint64_t>      (); }
uint32_t       BigUnsigned::toUnsignedInt32() const { return convertToPrimitive      <uint32_t>      (); }
uint16_t       BigUnsigned::toUnsignedInt16() const { return convertToPrimitive      <uint16_t>      (); }
int64_t        BigUnsigned::toInt64        () const { return convertToSignedPrimitive<int64_t>       (); }
int32_t        BigUnsigned::toInt32        () const { return convertToSignedPrimitive<int32_t>       (); }
int16_t        BigUnsigned::toInt16        () const { return convertToSignedPrimitive<int16_t>       (); }
...
void BigUnsigned::subtract(const BigUnsigned &a, const BigUnsigned &b) {
	...
	if (b.len == 0) {
        ...
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigUnsigned::subtract: "
	...
	if (borrowIn) {
		len = 0;
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigUnsigned::subtract: Negative result in unsigned calculation")
	} else
...
void BigUnsigned::divideWithRemainder(const BigUnsigned &b, BigUnsigned &q) {
	...
	if (this == &q)
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigUnsigned::divideWithRemainder: Cannot write quotient and remainder into the same variable")
...
void BigUnsigned::bitShiftLeft(const BigUnsigned &a, int b) {
	...
	if (b < 0) {
		if (b << 1 == 0)
            THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigUnsigned::bitShiftLeft: "
...
void BigUnsigned::bitShiftRight(const BigUnsigned &a, int b) {
	...
	if (b < 0) {
		if (b << 1 == 0)
            THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigUnsigned::bitShiftRight: "
                                                                   "Pathological shift amount not implemented")
		else {
			bitShiftLeft(a, -b);
			return;
...
void BigUnsigned::operator --() {
	if (len == 0)
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigUnsigned::operator --(): Cannot decrement an unsigned zero")
	Index i;
	bool borrow = true;
	for (i = 0; borrow; i++) {
		borrow = (blk[i] == 0);
