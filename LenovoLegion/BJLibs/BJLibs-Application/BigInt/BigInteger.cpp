...
BigInteger::BigInteger(const Blk *b, Index blen, Sign s) : mag(b, blen) {
	...
	case zero:
		if (!mag.isZero())        
            THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::BigInteger(const Blk *, Index, Sign): Cannot use a sign of zero with a nonzero magnitude")
		sign = zero;
		break;
	case positive:
	case negative:
	...
	default:
        ...
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::BigInteger(const Blk *, Index, Sign): Invalid sign")
	}
}
...
BigInteger::BigInteger(const BigUnsigned &x, Sign s) : mag(x) {
	...
	case zero:
		if (!mag.isZero())
            THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::BigInteger(const BigUnsigned &, Sign): Cannot use a sign of zero with a nonzero magnitude")
		sign = zero;
		break;
	case positive:
	case negative:
	...
	default:
		/* g++ seems to be optimizing out this case on the assumption
		 * that the sign is a valid member of the enumeration.  Oh well. */
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::BigInteger(const BigUnsigned &, Sign): Invalid sign")
	}
}
...
BigInteger::BigInteger(uint32_t       x) : mag(x) { sign = mag.isZero() ? zero : positive; }
BigInteger::BigInteger(uint16_t x      ) : mag(x) { sign = mag.isZero() ? zero : positive; }
...
BigInteger::BigInteger(int32_t    x) : sign(signOf(x)), mag(magOf<int32_t , uint32_t >(x)) {}
BigInteger::BigInteger(int16_t    x) : sign(signOf(x)), mag(magOf<int16_t  ,uint16_t >(x)) {}
...
X BigInteger::convertToUnsignedPrimitive() const {
	if (sign == negative)
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::to<Primitive>: Cannot convert a negative integer to an unsigned type")
	else
...
X BigInteger::convertToSignedPrimitive() const {
    ...
    THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::to<Primitive>: ""Value is too big to fit in the requested type")
}

uint64_t       BigInteger::toUnsignedInt64() const { return convertToUnsignedPrimitive<uint64_t >            (); }
uint32_t       BigInteger::toUnsignedInt32() const { return convertToUnsignedPrimitive<uint32_t >            (); }
uint16_t       BigInteger::toUnsignedInt16() const { return convertToUnsignedPrimitive<uint16_t>             (); }
int64_t        BigInteger::toInt64        () const { return convertToSignedPrimitive<int64_t,uint64_t>       (); }
int32_t        BigInteger::toInt32        () const { return convertToSignedPrimitive<int32_t,uint32_t>       (); }
int16_t        BigInteger::toInt16        () const { return convertToSignedPrimitive<int16_t,uint16_t>       (); }
...
BigInteger::CmpRes BigInteger::compareTo(const BigInteger &x) const {
	...
	default:
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger internal error")
	}
}
...
void BigInteger::divideWithRemainder(const BigInteger &b, BigInteger &q) {
	...
	if (this == &q)
        THROW_EXCEPTION(exception_T,ERROR_CODES::GENERIC_ERROR,"BigInteger::divideWithRemainder: Cannot write quotient and remainder into the same variable")
	if (this == &b || &q == &b) {
		BigInteger tmpB(b);
